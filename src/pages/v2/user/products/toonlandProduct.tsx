...
// ---- mock reviews (swap for real data whenever the API has it) ----
interface Review {
  id: string;
  name: string;
  rating: number;
...
// cycling ring colors for however many thumbnails come back
const RING_COLORS = ["#FF4B3E", "#2EC4B6", "#6C5CE7", "#FFC93C"];

// ---- shared comic-burst badge (the signature element) ----
const BURST_POINTS =
...
const Burst: ({ children, className, bg }: { children?: ReactNode; className?: string; bg?: string }) => JSX.Element = ({ children, className = "", bg = "#FF4B3E" }) => (
...
// (excludes the same types Mentoons excludes)
...
const ToonlandProductPage = () => {
  ...
  // The product is normally handed to us directly from
  // ProductDetailCards via `navigate(url, { state: { product } })` —
  // so on a normal click-through there's no network call at all.
  const passedProduct = (location.state as { product?: Products } | null)
    ?.product;
  ...
  // ---- "You will also like this" state (ported from Mentoons ProductDetails) ----
  ...
  // The recommendations rail and the "loaded product without router
  // state" fallback below both read from this same fetched list — no
  // separate call to the standalone Toonland backend is made anywhere
  // on this page.
  const recommendedProducts = allProducts;
//   const recommendedLoading = productsLoading;
  ...
  useEffect(() => {
    ...
    // No router state — e.g. the page was refreshed or opened via a
    // direct link. Fall back to finding the product in the products
    // already fetched into redux, instead of hitting a separate
    // Toonland backend.
    if (allProducts.length > 0) {
      const found = allProducts.find((p) => p._id === PRODUCT_ID);
      if (found) {
        setProduct(found as unknown as Products);
        setError(null);
      } else {
  ...
  }, [passedProduct, allProducts, PRODUCT_ID]);
    ...
    try {
      ...
      const response = await dispatch(
        addItemCart({
          ...
          // Toonland's Products type has no `type` field like Mentoons —
          // adjust this to whatever category value your cart/order backend expects
          productType: "comic",
          title: product.title,
          quantity: quantity || 1,
          price: product.offerPrice,
          // Toonland's Products type has no `ageCategory` field — placeholder
  ...
  return (
    <div className="min-h-screen bg-[#FFF6E9] px-4 py-10 sm:px-8 lg:px-16">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {/* ---------------- YOU WILL ALSO LIKE THIS (ported from Mentoons ProductDetails) ---------------- */}
